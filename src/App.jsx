...
function getSessionId() {
  let id = localStorage.getItem("nkarik_session");
  if (!id) { id = crypto.randomUUID(); localStorage.setItem("nkarik_session", id); }
  return id;
}
    ...
    <div className="magic-loader">
      <div className="wand-wrap">
        <div className="wand">
          ...
          {[...Array(6)].map((_,i) => <div key={i} className="wand-spark" style={{"--i":i}} />)}
        </div>
      </div>
...
function BadgePopup({ badges, onClose }) {
  ...
  return (
    <div className="badge-popup" onClick={onClose}>
      {badges.map(b => (
...
// ─── Guess bubble ─────────────────────────────────────────────────────────────
function GuessBubble({ guess, funFact, onCorrect, onWrong }) {
  if (!guess) return null;
  const label = guess.replace(/^a /, "").replace(/^an /, "");
  return (
    <div className="guess-bubble">
      <p className="guess-text">I think you drew… <strong>{label}</strong>? 🤔</p>
      <p className="guess-fact">{funFact}</p>
      <div className="guess-btns">
        <button className="guess-yes" onClick={onCorrect}>✅ Yes!</button>
        <button className="guess-no"  onClick={onWrong}>❌ Nope!</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [stats,         setStats]         = useState({ total:0, streak:0, badges:0 });
  ...
  const clearCanvas = useCallback(() => {
    ...
    ctx.fillStyle = "#fff";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    setShowHint(true); setError(null);
    saveHistory();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setGuess(json.guess);
      setFunFact(json.fun_fact);
      if (json.new_badges?.length) setNewBadges(json.new_badges);
  ...
  const handleConfirmGuess = async (correct) => {
    if (correct) {
      const res = await fetch(`${API_URL}/guess/confirm`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: sessionId.current, was_correct: true }),
      }).catch(() => null);
      if (res?.ok) {
        const json = await res.json();
        if (json.new_badges?.length) setNewBadges(b => [...b, ...json.new_badges]);
      }
    }
    setGuess(null);
  };
  ...
  const handleSave = () => {
    ...
    a.href = resultUrl; a.download = `nkarik-art-${Date.now()}.webp`; a.click();
  };

  const handleDrawAgain = () => {
    setScreen("draw"); setResultUrl(null); setError(null);
    setGuess(null); clearCanvas();
  };
      ...
      <div className="app result-screen">
        ...
        <div className="result-header">
          ...
          <div className="stats-row">
            <span>🎨 {stats.total} artworks</span>
        ...
        {guess && (
          <GuessBubble
            guess={guess}
            funFact={funFact}
            onCorrect={() => handleConfirmGuess(true)}
            onWrong={()  => handleConfirmGuess(false)}
          />
        )}
        ...
        <div className="result-actions">
          ...
          <button className="btn-secondary"
            onClick={() => navigator.share?.({ title: "My Nkarik art!", url: resultUrl })}>
            <span>📤</span> Share
          </button>
        </div>
    ...
    <div className="app draw-screen">
      ...
      <div className="toolbar card">
        ...
        <div className="toolbar-section tool-actions">
          <button className={`action-btn ${tool==="eraser"?"active":""}`}
            onClick={() => setTool(tool==="eraser"?"pen":"eraser")}>🧹 Eraser</button>
          <button className={`action-btn ${!canUndo?"disabled":""}`}
            onClick={undo} disabled={!canUndo}>↩️ Undo</button>
          <button className="action-btn danger" onClick={clearCanvas}>🗑️ Clear</button>
        </div>
      </div>
      ...
      <div className="creativity-card card">
        ...
        <div className="creativity-labels">
          <span>📐 Stay close</span><span>🚀 Go wild!</span>
        </div>
      </div>
